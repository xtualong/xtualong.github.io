...
$existing = $null
if (Test-Path $PROFILE) {
    $existing = [System.IO.File]::ReadAllText($PROFILE, [System.Text.Encoding]::UTF8)
}
if ($existing -and $existing -match "function pt") {
    ...
    if ($r -eq '' -or $r -eq 'Y' -or $r -eq 'y') {
        ...
        if ($endRel -ge 0) {
            ...
            [System.IO.File]::WriteAllText($PROFILE, $new, (New-Object System.Text.UTF8Encoding($false)))
    ...
    [System.IO.File]::AppendAllText($PROFILE, "`n$code", (New-Object System.Text.UTF8Encoding($false)))
    Write-Host "  完成" -ForegroundColor Green
}
